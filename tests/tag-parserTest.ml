...
(*I wrote - shahar*)
(*let vectorTests = fun () ->
    current_test := "vectorTests";
    test 1 (expr_eq (execute_tag_parse_expression (Vector
 [Number (Int 1);
  Pair (Symbol "quote",
   Pair (Pair (Number (Int 5), Pair (Number (Int 6), Nil)), Nil))])) (execute_expected(Const(Sexpr(Bool(true))))));
    ;; *)
